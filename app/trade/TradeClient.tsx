...
export default function TradeClient({ initialItems = [], initialDong = '개포동', tradeStats = null }: TradeClientProps) {
  ...
  useEffect(() => {
    ...
    if (pendingAptRef.current) {
      ...
      setKeyword(apt); // 카드 클릭 없이 자동 선택하면 카카오맵 SDK가 헤더 이벤트를 가로챔
    }
  }, [items]);
  ...
  return (
    <div>
      ...
      {/* ── 동네 분석 (개별 아파트 아래, 탭 공통) ── */}
      {lawdCd && (
        <>
          <DistrictTrendChart lawdCd={lawdCd} sigunguName={sigunguName} />
      ...
      {/* ── 청약·분양 배너 ── */}
      {sido && (
        <div style={{ marginTop: 8, background: '#1e3a5f', borderRadius: 16, padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
