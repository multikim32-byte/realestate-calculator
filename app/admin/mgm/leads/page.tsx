...
type MgmLead = {
  id: string; house_manage_no: string; name: string; birth_date: string;
  phone: string; address: string; memo: string | null; status?: string; created_at: string;
};
...
function MemoCell({ lead, onSave }: { lead: MgmLead; onSave: (id: string, memo: string) => void }) {
  ...
  async function save() {
    ...
    const res = await fetch('/api/mgm/leads', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: lead.id, memo: value }) });
    setSaving(false);
    if (res.ok) { onSave(lead.id, value); setEditing(false); } else setSaveError(true);
  }
  ...
  if (editing) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
        ...
        <button onClick={save} disabled={saving} style={{ padding: '5px 10px', borderRadius: 6, border: 'none', background: '#1d4ed8', color: '#fff', fontSize: 12, cursor: 'pointer' }}>{saving ? '…' : '저장'}</button>
        <button onClick={() => { setValue(lead.memo ?? ''); setEditing(false); setSaveError(false); }} style={{ padding: '5px 8px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', fontSize: 12, cursor: 'pointer' }}>취소</button>
      </div>
      {saveError && <span style={{ fontSize: 11, color: '#dc2626' }}>저장 실패.</span>}
    </div>
  );
...
export default function AdminMgmLeadsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<MgmLead | null>(null);
  ...
  useEffect(() => {
    fetch('/api/mgm/leads')
      ...
      .then(async (data: MgmLead[] | null) => {
        ...
        const results = await Promise.all(uniqueNos.map(no =>
          fetch(`/api/sale/detail?id=${no}`).then(r => r.json()).then(d => ({ no, name: d.item?.name ?? no })).catch(() => ({ no, name: no }))
        ));
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter(l => {
      ...
      if (search) {
        ...
        if (!l.name.toLowerCase().includes(q) && !l.phone.includes(q) && !l.address.toLowerCase().includes(q) && !aptName.includes(q)) return false;
      }
      return true;
    });
  }, [leads, search, dateFilter, statusFilter, aptNames]);

  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const paged = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);
  const grouped = paged.reduce<Record<string, MgmLead[]>>((acc, l) => {
    if (!acc[l.house_manage_no]) acc[l.house_manage_no] = [];
    acc[l.house_manage_no].push(l); return acc;
  }, {});
  ...
  const handleStatusChange = async (id: string, status: string) => {
    const res = await fetch('/api/mgm/leads', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, status }) });
  ...
  const handleDelete = async () => {
    ...
    const res = await fetch(`/api/mgm/leads?id=${deleteTarget.id}`, { method: 'DELETE' });
  ...
  const handleCSV = () => {
    const rows = [['아파트명', '성함', '생년월일', '전화번호', '거주지', '상태', '메모', '신청일시']];
    filtered.forEach(l => rows.push([aptNames[l.house_manage_no] ?? l.house_manage_no, l.name, l.birth_date, l.phone, l.address, STATUS_OPTIONS.find(s => s.value === (l.status ?? 'new'))?.label ?? '신규', l.memo ?? '', fmtDate(l.created_at)]));
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(',')).join('\n');
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob(['﻿' + csv], { type: 'text/csv;charset=utf-8;' }));
    a.download = `MGM리드_${new Date().toISOString().slice(0,10)}.csv`; a.click();
  };
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
              ...
              {STATUS_OPTIONS.map(s => (
                <span key={s.value} style={{ fontSize: 12, padding: '2px 8px', borderRadius: 10, background: s.bg, color: s.color, fontWeight: 700 }}>{s.label} {statusCounts[s.value] ?? 0}</span>
              ))}
            </div>
          </div>
          <button onClick={handleCSV} style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', fontSize: 13, cursor: 'pointer', color: '#374151', whiteSpace: 'nowrap' }}>CSV 다운로드</button>
        </div>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '16px 20px', marginBottom: 20, display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="이름·전화번호·거주지·아파트명 검색"
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {['전체', '오늘', '이번주', '이번달'].map(f => (
              <button key={f} onClick={() => { setDateFilter(f); setPage(1); }}
                style={{ padding: '6px 12px', borderRadius: 20, border: 'none', fontSize: 12, cursor: 'pointer', background: dateFilter === f ? '#7c3aed' : '#f1f5f9', color: dateFilter === f ? '#fff' : '#374151', fontWeight: dateFilter === f ? 700 : 400 }}>{f}</button>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {STATUS_OPTIONS.map(s => (
              <button key={s.value} onClick={() => { setStatusFilter(s.value); setPage(1); }}
                style={{ padding: '6px 12px', borderRadius: 20, border: 'none', fontSize: 12, cursor: 'pointer', background: statusFilter === s.value ? s.color : '#f1f5f9', color: statusFilter === s.value ? '#fff' : '#374151', fontWeight: statusFilter === s.value ? 700 : 400 }}>{s.label}</button>
            ))}
          </div>
          {(search || dateFilter !== '전체' || statusFilter !== '전체') && (
            <button onClick={() => { setSearch(''); setDateFilter('전체'); setStatusFilter('전체'); setPage(1); }}
              style={{ padding: '6px 12px', borderRadius: 20, border: '1px solid #fca5a5', background: '#fef2f2', color: '#dc2626', fontSize: 12, cursor: 'pointer' }}>필터 초기화</button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              {Object.entries(grouped).map(([no, groupLeads]) => (
                <div key={no} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                  <div style={{ background: '#fdf4ff', padding: '12px 20px', borderBottom: '1px solid #e9d5ff', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span style={{ fontSize: 15, fontWeight: 800, color: '#7c3aed' }}>{aptNames[no] ?? no}</span>
                      <span style={{ fontSize: 12, background: '#7c3aed', color: '#fff', padding: '2px 8px', borderRadius: 10, fontWeight: 700 }}>{groupLeads.length}건</span>
                    </div>
                    <Link href={`/sale/${no}`} target="_blank" style={{ fontSize: 12, color: '#6b7280', textDecoration: 'none' }}>청약 상세 →</Link>
                  </div>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: '#f8fafc' }}>
                        {['상태', '성함', '생년월일', '전화번호', '거주지', '메모', '신청일시', ''].map(h => (
                    ...
                    <tbody>
                      {groupLeads.map((lead, i) => (
                        <tr key={lead.id} style={{ borderBottom: i < groupLeads.length - 1 ? '1px solid #f3f4f6' : 'none', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                          <td style={{ padding: '8px 16px', whiteSpace: 'nowrap' }}><StatusBadge lead={lead} onChange={handleStatusChange} /></td>
                          ...
                          <td style={{ padding: '10px 16px', whiteSpace: 'nowrap' }}><a href={`tel:${lead.phone}`} style={{ color: '#1d4ed8', textDecoration: 'none', fontWeight: 600 }}>{lead.phone}</a></td>
                          <td style={{ padding: '10px 16px', color: '#374151', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{lead.address}</td>
                          <td style={{ padding: '8px 16px' }}><MemoCell lead={lead} onSave={(id, memo) => setLeads(prev => prev.map(l => l.id === id ? { ...l, memo } : l))} /></td>
                          <td style={{ padding: '10px 16px', color: '#9ca3af', whiteSpace: 'nowrap' }}>{fmtDate(lead.created_at)}</td>
                          <td style={{ padding: '10px 16px', whiteSpace: 'nowrap' }}>
                            <button onClick={() => setDeleteTarget(lead)} style={{ padding: '4px 10px', borderRadius: 6, border: 'none', background: '#fef2f2', color: '#dc2626', fontSize: 12, cursor: 'pointer' }}>삭제</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, marginTop: 28 }}>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).filter(p => Math.abs(p - page) <= 2 || p === 1 || p === totalPages).map((p, i, arr) => (
                  <span key={p}>
                    ...
                    <button onClick={() => setPage(p)} style={{ width: 36, height: 36, borderRadius: 8, border: 'none', background: page === p ? '#7c3aed' : '#fff', color: page === p ? '#fff' : '#374151', fontSize: 13, fontWeight: page === p ? 700 : 400, cursor: 'pointer' }}>{p}</button>
                  </span>
                ))}
