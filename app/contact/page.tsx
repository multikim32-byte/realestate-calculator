...
const SERVICES = [
  ...
  { label: '전국 부동산 지도', desc: '전국 미분양·분양 단지를 카카오맵 위에서 위치 기반으로 탐색' },
  { label: '지역별 모아보기', desc: '시도별 청약 일정, 분양 매물, 실거래가를 한 페이지에서 확인' },
  { label: '관심단지',   desc: '청약·분양 단지를 저장해두고 빠르게 다시 확인' },
  { label: '카카오 채널 알림', desc: '이번 주 청약 소식·실거래 트렌드를 카카오톡으로 수신 (채널 친구 추가 시)' },
];
...
const FAQS = [
  ...
  },
  {
    ...
    a: '청약정보는 국토교통부 청약홈 공공데이터 API를 통해 실시간으로 제공됩니다. 분양정보(미분양 특별 혜택 단지)는 단지집사 자체 DB로 관리되며 수시로 업데이트됩니다.',
  },
  {
